...
| Bankr skill | [`skills/lp-truth/`](skills/lp-truth/SKILL.md). PRs to BankrBot/skills: `lp-truth` {{LP_TRUTH_PR_URL}}, `hood-stock-lp ledger` {{HOOD_LEDGER_PR_URL}}, `aero-stock-lp ledger` {{AERO_LEDGER_PR_URL}} |
...
| The user's lane (lane A, NVDA/USDG) | {{LANE_ADDRESS}}, created by the user's Vault in {{CREATE_LANE_TX}} and funded in {{LANE_FUNDING_TXS}} |
| The agent's on-chain decision | A delegated `signal()` `LaneAction` for the weekend gate: {{SIGNAL_TX}} |
| Dynamic policy denial of a staged malicious transfer request (`USDG.transfer(owner, 1)`, the call a prompt injection would try), sent straight to the delegated signer | {{POLICY_DENIAL_LOG}} |
...
- **Dynamic.**
  ...
  - The agent's decision becomes a transaction on 4663: {{SIGNAL_TX}}.
  - A policy denial on camera, of a staged malicious transfer request: {{POLICY_DENIAL_LOG}}.
