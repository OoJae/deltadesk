// DeltaDesk x402 handler: historical LP edge by regime and hour of week. Proxies to the DeltaDesk API with the premium key.
...
export default async function handler(req: Request) {
  ...
  const upstream = await fetch(`${API}/pool-toxicity/${pool}`, {
